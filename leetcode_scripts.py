...
def judgeSquareSum(c: int) -> bool:
    ...
    # Way #2
    a = floor(sqrt(c))
    full_range_counter = a
    loop_counter = 1

    while True:
        if (full_range_counter ** 2 + loop_counter ** 2) == c:
            return True
        full_range_counter -= 1
        if full_range_counter == 0:
            loop_counter += 1
            full_range_counter = a
...
def isHappy(n: int) -> bool:
    ...
    while goal > 1 and iter_count < 1000:
        n_digits = [int(x) for x in str(goal)]
        if len(n_digits) == 1:
            n_digits.insert(0, 0)
        goal = sum([x ** 2 for x in n_digits])
        iter_count += 1
...
def getLucky(s: str, k: int) -> int:
    lo_alpha = ascii_lowercase

    count = 1
    alpha_values = {}

    for item in lo_alpha:
        alpha_values[item] = count
        count += 1
    ...
    for letter in s:
        conv_str += str(alpha_values[letter])

    while control_count > 0:

        new_str_num = sum([int(x) for x in conv_str])
        conv_str = str(new_str_num)
        control_count -= 1
    return conv_str
...
def countEven(num: int) -> int:
    ...
    for numbers in range(num):
        even_list = [int(x) for x in str(count)]
...
def addDigits(num: int) -> int:
    ...
    while check_value > 10:
        check_value = sum([int(x) for x in str(check_value)])
